...
DmxOutput::return_code DmxOutput::begin(uint pin, uint prgm_offset, PIO pio ,bool inverted)
{
    _inverted = inverted;
    /* 
    ...
    */
    ...
    if (inverted) 
        pio_sm_init(pio, sm, prgm_offset + 8, &sm_conf);
    else
...
void DmxOutput::write_dmx(uint8_t *universe, uint length)
{
    ...
    if (_inverted)
        pio_sm_exec(_pio, _sm, pio_encode_jmp(_prgm_offset + 8));
    else
