    ...
    if (p->tot_len > 0) {
        ...
        if (strncmp(HTTP_GET, con_state->headers, sizeof(HTTP_GET) - 1) == 0) {
            ...
            if (con_state->result_len > 0) {
                ...
                // Send redirect
                con_state->header_len = snprintf(con_state->headers, sizeof(con_state->headers), NULL,
...
static err_t tcp_server_accept(void *arg, struct tcp_pcb *client_pcb, err_t err) {
    ...
    TCP_CONNECT_STATE_T *con_state = (TCP_CONNECT_STATE_T *)malloc(sizeof(TCP_CONNECT_STATE_T));
    if (!con_state) {
